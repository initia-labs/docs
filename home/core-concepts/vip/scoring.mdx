...
To score users, the rollup operator must first register the `deployer` account on the `vip_score` contract using their operator key. Once registered, the deployer can add and remove user scores based on the determined criteria. All future scoring actions can only be performed by the deployer until they are changed or removed.
