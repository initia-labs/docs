...
<Tip>
For a more technical overview of Minitias and its architecture, please refer to the [Technical Architecture](/home/technical-architecture/initia-l1/introduction) section.
</Tip>
...
At the heart of the Initia ecosystem lies the Initia L1, a custom-built Layer 1 blockchain developed on the [Cosmos SDK](https://docs.cosmos.network). This layer serves as the coordination hub for all Minitias, offering crucial services:

1. **Security**: Minitia rollups leverage Initia L1 and its validator network for:
    - Secure token bridging (INIT tokens locked in L1 module when bridged to Minitias)
    - Settlement layer for state transitions

2. **Liquidity**: The [InitiaDEX](/home/core-concepts/initiadex) on L1 acts as:
    - Central liquidity hub for Minitias
    - Platform for building and incentivizing token liquidity
    - Facilitator for cross-Minitia bridging and swaps

3. **Interoperability**: L1 functions as:
    - Central hub and router for Minitias
    - Single connection point enabling communication with L1, other Minitias, and external blockchain ecosystems

4. **Incentives**: L1 powers various incentive schemes:
    - Vested Interest Program (VIP): Encourages user actions beneficial to Minitia applications
...
3. **Inter-Minitia Router**:
    - Seamless token movement and swaps between Minitias
...
1. **Fraud Proofs and Challenges**:
    - Minitias utilize Initia L1's validator set and governance
...
2. **Rollbacks**:
    ...
    - Ensures integrity of Minitia states
