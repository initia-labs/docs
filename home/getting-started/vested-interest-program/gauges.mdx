...
1. Voting Power Snapshot Submission
2. Gauge Voting

### Voting Power Snapshot Submission

Before the gauges can be voted on for a given epoch, a governance-assigned party known as the Snapshot Submitter first snapshots and submits the voting power of all of the individual stakers on the Initia L1 at a certain block height.

### Gauge Voting Process on Initia L1

Once the voting power snapshot is made, the gauge voting process begins. Each VIP-whitelisted minitia has a "gauge" that determines the ratio of the total Weight Pool rewards they are eligible to receive. Stakers with staking tokens on Initia L1 use their voting power to increase the weight of the gauges for the minitias they prefer. Stakers can allocate their entire voting power to a single minitia's gauge or spread it across multiple gauges.
...
In this case, the Weight Pool rewards for the epoch will then be split 55:40:5.
...
Within an epoch, a challenge proposal can be made any time after the original Snapshot Submission, while leaving enough time for the challenge proposal to be voted on.


For example, given
-  Epoch duration: $10$ units
...
Once submitted, users can start voting on the challenge proposal. Eligible voters include those who participated in gauge voting during the previous epoch, with each voter's voting power on the challenge proposal equal to the total voting power they used to vote on the gauges during the previous epoch. 

For example, if a user has 100 total voting power in the previous epoch but only 50 of it was used to vote on the gauges, their voting power on the challenge proposal would be 50.
...
1. the snapshot in the challenge proposal is used for this epoch
