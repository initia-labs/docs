...
At network genesis, a portion of the total INIT token supply will be set aside as rewards for the VIP program, which will be distributed over several years. Let's call this portion of the supply $R$. These rewards are given out at regular intervals, or **epoch** $t$, throughout the distribution period.
...
At each rewards distribution, epoch $t$, the total VIP rewards for that stage $R_t$ are distributed amongst each minitia ecosystem. The distribution is split in multiple steps:

1. Splitting the total rewards allocated for that epoch, $R_t$, between all whitelisted minitias
...
These two pools aim to incentivize and reward different aspects of minitia operations, which are described in the sections below. The ratio between $R_B$ and $R_W$, called `poolSplitRaio` is set by the Initia L1 governance and can be changed through on-chain proposals, and its sum follows the following equation:
...
The Weight Pool ($R_W$) rewards minitias based on their weight value, which is determined by a [gauge weight voting](/home/getting-started/vested-interest-program/gauges) process similar to protocols such as Curve's. 

For a given minitia$m$, the rewards from the Weight Pool during an stage are calculated as:
...
Once we know how much rewards each minitia receives for a given stage, we then further distribute these rewards to the users of the minitia. The amount of VIP rewards to each user is defined by a scoring methodology decided by the minitia team. The scoring method could be based factors such as:
...
or the combination of any multiple metrics. At the end of each stage, the rewards for each user are calculated and stored. Once the rewards for each minitia are known, the rewards are then directly distributed to the users of the minitia.
...
To ensure that minitia operators are also incentivized to keep their minitias active and useful, they can set a `operatorCommissionRate`,$c$. This rate allows them to take a portion of the rewards for their minitia.

//TODO: Diagram

The rewards for an operator $o$ of minitia $m$ during an stage are:

$$
R_{o,m,t} = R_{m,t} c_{m}
$$

The commission rate is set when the minitia is first approved for VIP rewards and can be changed later, but only by a fixed amount amount each stage to prevent abuse.
...
-  Vesting period ($p$): 52 stages
-  Stage duration ($v$): 7 days
-  Minimum score factor ($f$): 0.5
-  Total INIT amount to be vested: 100

To receive the full $\frac{100}{52}$ INIT tokens at each stage, the user must achieve a score of at least$100 \cdot f = 50$ for the next 52 stages.
