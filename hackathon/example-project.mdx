...
<Tip>
`rev = "main"` always pulls the latest version of the stdlib, which is appropriate for Initia
rollups tracking the current chain version. If you hit a compilation error after pulling fresh
dependencies, your `initiad`/`minitiad` binary may be out of sync with the stdlib. Run
`weave update` or re-install `minitiad` to match the chain's current version.
</Tip>
