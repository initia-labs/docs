---
title: 'Case Study: Lottery Appchain - Move'
icon: ticket
---
...
[reference repo](https://github.com/yonnie-park/side-quest/tree/main/move/sources).
</Note>
...
```toml title="move/Move.toml"
[package]
name = "lotteria"
version = "0.0.0"

[dependencies]
InitiaStdlib = { git = "https://github.com/initia-labs/move-natives.git", subdir = "initia_stdlib", rev = "v1.1.10" }

[addresses]
lottery = "0x<YOUR_HEX_ADDRESS>"
std = "0x1"
```
...
<Warning>
This example is pinned to `v1.1.10`, which matches the local rollup version used in this guide.
Using a newer stdlib revision can still compile but fail during `minitiad move deploy`. If your
toolchain is on a different Initia release, update `rev` to the matching stdlib version before
deploying.
</Warning>
...
public entry fun buy_ticket(
    ...
    // Transfer tokens from buyer to the module address
    primary_fungible_store::transfer(account, fa_metadata, @side_quest, amount);
...
<Note>
`fa_metadata` is the `Object<Metadata>` handle for the fungible asset accepted as payment
(e.g., the INIT token on your rollup). The actual token transfer is performed by
`primary_fungible_store::transfer` — without this call, no funds move onchain. Refer to the
[full source](https://github.com/yonnie-park/side-quest) for how the metadata object is
resolved at runtime.
</Note>
...
<Warning>
The prop is `chainId`, not `defaultChainId`. Passing an unrecognized prop is silently ignored,
and the provider will fall back to `interwoven-1` (Initia mainnet). Your local rollup
transactions will fail to broadcast.
</Warning>
...
<Note>
`@initia/utils` is not a published npm package. The `truncate` helper shown in the Side Quest
source is an internal utility. The inline version above is equivalent — or copy it directly
from the [reference repo](https://github.com/yonnie-park/side-quest).
</Note>
...
<Note>
The request body must include both `type_args` and `args`, even when both are empty. Omitting
`type_args` will return a 400 Bad Request error from the LCD endpoint.
</Note>
