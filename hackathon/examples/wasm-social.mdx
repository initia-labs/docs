...
If the `code_id` is not returned directly, wait for 5 seconds for indexing and then query the transaction hash:
...
const Board = () => {
  ...
  const fetchMessages = async () => {
    try {
      ...
      const queryData = Buffer.from(JSON.stringify({ all_messages: {} })).toString('base64')
      ...
      {!initiaAddress ? (
        ...
        <div className="auth-section">
          <div className="wallet-info">
            ...
            <button onClick={() => disconnect()} className="btn-secondary" style={{ marginLeft: '12px' }}>
              Disconnect
            </button>
          </div>
        </div>
...
To make your MemoBoard "Interwoven-native," you can replace long, complex
addresses with human-readable **Initia Usernames** (e.g., `vitalik.init`).

## Step 6: Register your .init Name

Before updating your code, you should register a primary username for your wallet
on the Initia testnet.

1.  Navigate to [app.testnet.initia.xyz/usernames](https://app.testnet.initia.xyz/usernames)
...
Your AI assistant knows how to integrate Initia Usernames. Simply ask it to
update your board.
