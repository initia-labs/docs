...
const Board = () => {
  ...
  const { initiaAddress, requestTxBlock, openWallet, openConnect, username } =
    useInterwovenKit()
...
### Step 0: Register your .init Name
...
The `useInterwovenKit()` hook provides the `username` for the currently connected wallet. You can use it to personalize the UI:
...
<Tip>
  **Pro Tip:** For other addresses in the feed, continue using the truncated hex address until a public multi-address resolver is available.
</Tip>
</Accordion>
