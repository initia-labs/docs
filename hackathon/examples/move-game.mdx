...
# This tutorial deploys from gas-station, so the module owner is gas-station's bech32 address.
...
Render `<Game />` from `src/App.jsx`.
  ...
  <Tip>
    **Pro Tip: Move Frontend Address Rules:** For `MsgExecute`, `moduleAddress`
    must stay **bech32**. For `rest.move.resource`, the struct tag must use the
    module address in **hex**. Using `init1...::items::Inventory` will fail
    with `invalid struct tag`.
  </Tip>
...
3. Connect your wallet in `blockforge-frontend`.
4. Click `Mint Shard` three times.
5. Confirm the displayed shard/relic counts update.
6. Click `Craft Relic`.
7. Confirm the inventory updates to reflect consumed shards and one relic.
