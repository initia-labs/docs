...
<Tip>
  **Pro Tip:** For a cleaner UI, hide the numeric "up/down" arrows in your CSS:
  ...
  `myBalance`), you MUST provide a `from` address in your `eth_call` parameters
  so the appchain knows whose balance to check.
  This tutorial uses an EVM appchain with 18 decimals, so `parseEther` and
  `formatEther` are appropriate. If your chain uses different decimals, switch
  to `parseUnits` and `formatUnits` with your runtime decimals value.
</Tip>
...
const NATIVE_DENOM = '<YOUR_NATIVE_DENOM>'
const NATIVE_SYMBOL = '<YOUR_NATIVE_SYMBOL>'
...
3. Connect your wallet in `minibank-frontend`.
4. Deposit `1` token from the UI and confirm the wallet prompt.
5. Confirm the displayed savings updates.
6. Withdraw `0.5` token from the UI.
7. Confirm the displayed savings updates again.
