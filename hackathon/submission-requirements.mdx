---
title: 'Submission Requirements'
icon: check-to-slot
---

Use this page to prepare your final submission. Complete the required files,
fill in every field correctly, and make sure your repository includes real
deployment evidence.

<Note>
**Showcase your app's originality:** Projects that only reproduce an example
without meaningful customization are unlikely to score as well as projects with
clear custom logic, UX, or product differentiation.
</Note>

## Create `.initia/submission.json`

Use this exact structure:

```json
{
  "project_name": "My Project",
  "repo_url": "https://github.com/<org>/<repo>",
  "commit_sha": "0123456789abcdef0123456789abcdef01234567",
  "rollup_chain_id": "my-game-1",
  "deployed_address": "0x...",
  "vm": "move",
  "native_feature": "auto-signing",
  "core_logic_path": "blockforge/sources/items.move",
  "native_feature_frontend_path": "blockforge-frontend/src/Game.jsx",
  "l2_interaction_tx": "0x...",
  "demo_video_url": "https://youtu.be/..."
}
```

## Submission JSON field guide

All fields in `.initia/submission.json` are required.

| Field | Expected value |
| :---- | :------------- |
| `project_name` | Non-empty string |
| `repo_url` | Reachable public GitHub repository URL |
| `commit_sha` | 40-character hex Git commit SHA |
| `rollup_chain_id` | Non-empty string |
| `deployed_address` | Primary deployed address for your application logic. Use your contract address, or your module address for Move projects |
| `vm` | `move`, `evm`, or `wasm` |
| `native_feature` | `auto-signing`, `interwoven-bridge`, or `initia-usernames` |
| `core_logic_path` | Repo-relative file path that must exist at `commit_sha` |
| `native_feature_frontend_path` | Repo-relative file path that must exist at `commit_sha` |
| `l2_interaction_tx` | Real, non-empty transaction hash or explorer URL |
| `demo_video_url` | Public Loom or YouTube URL for a 1 to 3 minute walkthrough video |

## Create `README.md`

Your README should include:

- a short project overview
- a brief explanation of your custom implementation
- a brief explanation of the native feature you used
- simple local run instructions for judges

Use this example block near the top of your `README.md`:

```markdown
## Initia Hackathon Submission

- **Project Name**: [Your Project Name]
- **Submission JSON**: [Link to `.initia/submission.json` in this repo]

### Project Overview

[Provide a 2-3 sentence description of your application. Why is it valuable and
what problem does it solve?]

### Implementation Detail

- **The Custom Implementation**: Briefly describe the unique logic you added.
  What original functionality did you design and implement?
- **The Native Feature**: Which Interwoven feature did you use, and exactly how does
  it improve the user experience?
- **The Value Add**: What meaningful value does this application create for
  crypto users, and how does it specifically strengthen the Initia ecosystem?

### How to Run Locally

[Provide 3-4 clear steps for a judge to run your frontend and connect it to a
local environment if necessary.]
```

## Final Check Before You Submit

- `.initia/submission.json` exists and all fields are complete
- `README.md` explains the project and how to run it
- `core_logic_path` and `native_feature_frontend_path` point to real files
- `deployed_address`, `rollup_chain_id`, and `l2_interaction_tx` are real and verifiable
- `demo_video_url` is public and clearly shows your app in action
