...
The saved-view pattern is VM-agnostic, so you can implement the same registry
on MiniMove or MiniWasm by using the equivalent Move or CosmWasm syntax.
...
Use `useInterwovenKit` for the write path and `useHexAddress` for the read.
The component loads the user's saved views on mount, lets them open any saved
view to see the matching Dune result, and refreshes the list after a new
preference is saved.
