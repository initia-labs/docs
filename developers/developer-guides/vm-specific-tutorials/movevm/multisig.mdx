...
In this tutorial, we will explore how to create and manage multisignature
...
const msgVoteProposal1 = new MsgExecute(
  ...
  'vote_proposal',
  [],
  [bcs.address().serialize(multisigAddress), bcs.u64().serialize(1), bcs.bool().serialize(true)].map((v) =>
    v.toBase64(),
  ),
)
```
...
const msgExecuteProposal = new MsgExecute(
  ...
  'execute_proposal',
  [],
  [bcs.address().serialize(AccAddress.toHex(multisigAddress)), bcs.u64().serialize(proposalId)].map((v) =>
    v.toBase64(),
  ),
)
```
