...
<Tabs>
  ...
  const restClient = new RESTClient('https://rest.initation-2.initia.xyz', {
  ...
  const accounts = keys.map((mnemonic) => new Wallet(restClientClient, new MnemonicKey({ mnemonic })))
