...
<Tab title="Initia L1">
```bash
initiad query bank balances [your-address] --node https://rpc.testnet.initia.xyz
```
</Tab>
<Tab title="Move Rollups">
```bash
minitiad query bank balances [your-address] --node [your-rollup-rpc-url]
```
</Tab>
</Tabs>

<Note>
For Move rollups, replace `[your-rollup-rpc-url]` with your specific rollup's RPC endpoint. You can find rollup-specific RPC URLs in the [Initia Registry](https://github.com/initia-labs/initia-registry/tree/main) or your rollup's documentation.
</Note>
</Step>
</Steps>
