...
If you want to update MiniWasm ACL, you need to use `MsgExecuteMessages` in OPchild module.
...
```bash
curl -X GET "https://lcd.miniwasm-1.initia.xyz/initia/ibchooks/v1/acls" -H "accept: application/json"
```
...
```typescript
import { Coin, Height, LCDClient, MnemonicKey, MsgTransfer, sha256, Wallet } from "@initia/initia.js";
...
async function main() {
    const l1lcd = new LCDClient('https://lcd.initiation-1.initia.xyz', {
        gasAdjustment: '1.75',
        gasPrices: '0.15uinit' // set l1 gas price
    })
    ...
    const tokenPair = await l1lcd.ophost.tokenPairByL1Denom(2, "uinit") // { l1_denom: 'uinit', l2_denom: 'l2/2588fd87a8e081f6a557f43ff14f05dddf5e34cb27afcefd6eaf81f1daea30d0' }
