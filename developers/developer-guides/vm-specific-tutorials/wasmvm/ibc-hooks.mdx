...
- `Funds`: This field is set to the amount of funds being sent over in the ICS
  ...
  chains representation of the denom, so we have to translate it to Initia's
  representation.

<Note>
  Due to a [bug](https://x.com/SCVSecurity/status/1682329758020022272) in the
  packet forward middleware, we cannot trust the sender from chains that use
  PFM. Until that is fixed, we recommend chains to not trust the sender on
...
this can be used for phishing easily. So, we need to set the ACL for the
contract to prevent unauthorized access. To update MiniWasm ACL, you need to use
