...
To query prices from Slinky oracle on WasmVM, you need to utilize the queries provided by the [wasm-slinky-query](https://github.com/initia-labs/wasm-slinky-query) contract. Specifically, the contract provides 3 queries:
...
To make the queries from your own contracts, you can copy the `packages` folder found in the wasm-slinky-query contract repository into your own project. This folder contains the necessary files and definitions to make the queries from your own contracts.


See [here](https://github.com/initia-labs/examples/wasmvm) for an example contract that integrated the queries.
...
This query takes two arguments: `base` and `quote`, which represent the base and quote asset symbol strings of the asset pair you want to query. Note that the `base` and `quote` pair must be supported by Slinky and be included in the `GetAllCurrencyPairsResponse`. For example, "BTC" and "USD" is a valid pair, but "BTC" and "AAVE" is not.
