...
Before diving into the IBC metadata format, take a look at the hook data
format and the fields that need to be set. The EVM `MsgCall` is
defined
...
An ICS20 packet is considered directed towards evmhooks iff all of the
following hold:
...
Ack callbacks. The contract that wants to receive ack callbacks must
