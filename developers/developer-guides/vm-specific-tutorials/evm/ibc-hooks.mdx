...
- `Sender`: We cannot trust the sender of an IBC packet, the counter-party chain
  ...
  with an account to represent the sender prefixed by the channel and a evm
...
- [x] `memo["evm"]["message"]` has exactly five entries, `"contract_addr"` and
      `"input"`
- [x] `receiver` == "" || `receiver` ==
...
This tutorial will guide you through the process of deploying a EVM contract and
calling it from another chain using IBC hooks. We will use IBC hook from Initia
chain to call a EVM contract on MiniEVM chain in this example.
