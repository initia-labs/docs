...
<Warning>
  Never share your `.env` file or private key. Anyone with this key can control your account. Keep it local and **add
  `.env` to your `.gitignore`** so it is never committed to Git.
</Warning>
...
<Note>
  You can also find factory addresses on the [Networks](/resources/developer/initia-l1) page or by calling the
