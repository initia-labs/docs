 ...
 * Note: This requires the mnemonic for the chain’s operator(a.k.a bridge executor), which you must obtain from the chain administrator.
 */
async function main() {
  /**
   ...
   *      presumably the authorized executor or has the necessary roles on the chain.
   */
  const wallet = new Wallet(
    ...
    new MnemonicKey({
       ...
       */
      mnemonic: "" // enter executor mnemonic
    })
  );
   ...
   *    - The first argument is the wallet’s (executor’s) account address.
   ...
   *      and then sign it using the wallet’s (executor’s) private key.
   */
