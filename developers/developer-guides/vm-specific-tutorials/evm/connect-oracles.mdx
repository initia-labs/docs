...
We then update the contract from the template to be our oracle contract.
...
Now that Our contract is compiled and ready, we can deploy it to the MiniEVM. To accomplish this, we will use Foundry's `forge deploy` command.  First, we need to create a script file that will handle the deployment. Create a new file named `Oracle.s.sol` in the `script` directory.
