---
title: Introduction
description: Learn how to install and get started with the minitiad command-line interface
  for interacting with Initia rollups
---
...
<Info>
  minitiad is built using the Cosmos SDK and provides a familiar interface for users of other Cosmos-based chains,
  specifically optimized for Initia rollups.
</Info>
...
<Tip>Check your rollup chain's documentation to determine which VM type it uses before installation.</Tip>
...
<Tip>You can verify your Go installation by running `go version` in your terminal.</Tip>
...
<Tab title="Wasm">```bash git clone https://github.com/initia-labs/miniwasm.git cd miniwasm ```</Tab>
...
<Note>The CLI binary is always named `minitiad` regardless of which VM-specific repository you cloned.</Note>
...
<Warning>
  Make sure to use the correct endpoints for your specific rollup chain. Each Initia rollup has its own unique endpoints
  and chain ID.
</Warning>
...
<Info>Additional minitiad CLI documentation pages are currently being developed and will be available soon.</Info>
