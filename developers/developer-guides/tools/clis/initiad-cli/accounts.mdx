...
<Note>
  Even with the same mnemonic phrase, the derived addresses differ because the method of generating the public key
  varies, leading to each account being treated separately.
</Note>
...
<Note>
  The exported private key is encrypted with a passphrase. So, you can only import it using `initiad` CLI. It is not
  possible to import it directly on Wallet Apps.
</Note>
