...
<Warning>
`Weave` only supports IBC relayer setup between Initial L1 and Interwoven Rollup(s). Setting up relayers between other arbitrary networks is not supported.
</Warning>

<Note>
Currently, `Weave` exclusively supports the `Hermes` relayer. For detailed information about Hermes, please refer to the [Hermes documentation](https://github.com/informalsystems/hermes).
</Note>
