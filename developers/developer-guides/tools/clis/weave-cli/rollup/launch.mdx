...
<Note>
  Weave will send some funds from Gas Station to the OPinit Bot accounts during this process. Please make sure that your Gas Station account has enough funds (6 INIT + 2 INIT or TIA depending on the DA option you choose) to cover the total amount of funds to be sent (this amount will be shown to youbefore sending the funds).
</Note>
...
To launch from the [config file](/nodes-and-rollups/deploying-rollups/rollup-deployment/running-the-rollup#preparing-the-configuration-file) without going through the interactive setup process, use the `--with-config` and `--vm` flags.
```bash
...
<ParamField path="--with-config" type="string">
  Path to the rollup [config file](/nodes-and-rollups/deploying-rollups/rollup-deployment/running-the-rollup#preparing-the-configuration-file). Use this flag to launch from a config file without going through the interactive setup process.
</ParamField>
