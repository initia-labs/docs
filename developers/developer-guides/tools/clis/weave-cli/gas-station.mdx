...
It distributes funds to essential services like [OPinit Bots](/nodes-and-rollups/deploying-rollups/opinit-bots/introduction) (including bridge executor, output submitter, and batch submitter) 
and the [IBC relayer](https://tutorials.cosmos.network/academy/2-cosmos-concepts/13-relayer-intro.html) to ensure smooth operation of the network.
...
Then fund the account with sufficient `INIT` tokens to support the necessary components. If you're planning to use `Celestia` as your Data Availability Layer, you'll also need to fund the account with `TIA` tokens.
