...
InitiaJS also provides a `Wallet` class that simplifies the process of creating, signing, and broadcasting transactions. This section will provide examples on how to use do each of these actions.
...
- `LCD_URL`: The URL of the Initia LCD.
...
- We then broadcast the transaction by calling `lcd.tx.broadcast`. This function takes in the signed transaction object and broadcasts it to the network.
...
If you then run the script using `npx ts-node src/transactions.ts`, you should see an output similar to the following:
