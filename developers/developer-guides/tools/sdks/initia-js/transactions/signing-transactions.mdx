...
- `RESTClient`: A class that represents a LCD client.
...
To start, we will create two `RESTClient` instances. One that is connected to the network, and one that is offline. We will be using the offline LCD client to sign the transaction, and the online LCD client to broadcast the transaction to later compare.

```ts
const restOffline = new RESTClient(``, {
  chainId: chainId,
  gasPrices: gasPrices,
});

// rest client that is connected
const restOnline = new RESTClient(restUrl, {
  chainId: chainId,
  gasPrices: gasPrices,
});
```
...
const account = await restOnline.auth.accountInfo(key.accAddress); // you have to know account number and current sequence
const wallet = new Wallet(restOffline, key);
...
const feeAmount = new Coins(restOffline.config.gasPrices)
  .toArray()[0]
  .mul(gasLimit);
...
Finally, if we want to broadcast the transaction to the network later, we can do so by using the `restOnline` client. If we then retrieve the transaction hash from the online LCD client and compare it to the transaction hash we computed earlier, we should see that they match.

```ts
const broadcastRes = await restOnline.tx.broadcastSync(signedTx);
