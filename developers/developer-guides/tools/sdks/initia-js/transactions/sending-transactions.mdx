...
Sending transactions using InitiaJS is straightforward. First, we need to import
...
We can then use these variables to ultimately create the `wallet` client, as
well as the `sendMsg` object. Since for this example we're making a token
...
- We first create the transaction by calling `wallet.createAndSignTx`. This
  function takes in the raw message object, signs it using the `wallet`, and
  returns a signed transaction object.
- We then broadcast the transaction by calling `restClient.tx.broadcast`. This
