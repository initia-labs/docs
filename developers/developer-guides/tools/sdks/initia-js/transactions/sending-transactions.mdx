...
const rest: RESTClient = new RESTClient(restUrl, { gasPrices });
const wallet: Wallet = new Wallet(rest, key);
...
- We then broadcast the transaction by calling `rest.tx.broadcast`. This function takes in the signed transaction object and broadcasts it to the network.
...
async function sendTransaction(): Promise<WaitTxBroadcastResult> {
  try {
    ...
    const result: WaitTxBroadcastResult = await rest.tx.broadcast(signedTx);
