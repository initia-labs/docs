...
const restUrl = 'https://rest.testnet.initia.xyz';
const rest: RESTClient = new RESTClient(restUrl, {
    chainId: '[chain-id]',
...
async function queryBalance(address: string) {
  ...
  do {
    const [coins, pagination] = await rest.bank.balance(address, { 'pagination.key': nextKey || undefined });
...
```typescript
const blockInfo = await rest.tendermint.blockInfo(10000) // If no height is given, the latest block is returned. 
```
- `txInfo()`: query the transaction information
```typescript
const txInfo = await rest.tx.txInfo('6DFEE8E4BFC38341E8AADBD74A23588D8DE94FA38052CB5721DDA780A24F8B1D')
```
- `price()`: query the oracle price
```typescript
const currenyPair = new CurrencyPair('BTC', 'USD')
const price = await rest.oracle.price(currenyPair)
```
...
```typescript
const res = await rest.move.viewJSON(
    '0x1',                                               // owner of the module
    'object',                                            // name of the module
    'owner',                                             // function name
    ["0x1::object::ObjectCore"],                         // type arguments
...
```typescript
const resources = await rest.move.resources('0x1')
const resource = await rest.move.resource('0x1', '0x1::code::ModuleStore')
```

- `modules()`: query the move contract modules
```typescript
const modules = await rest.move.module('0x1')
const module = await rest.move.module('0x1', 'object')
```
...
const tableInfo = await rest.move.tableInfo(tableHandle)
const tableEntries = await rest.move.tableEntries(tableHandle)
const tableEntry = await rest.move.tableEntry(tableHandle,entryKeyBytes)
```
...
```typescript
const contractInfo = await rest.wasm.contractInfo('init14mv62l7x4ducykg0crfa9a22egf8yrltmxzy84zn0wqgmr496jqs5z7k0c')
const contracts = await rest.wasm.contractsByCode(1) 
```

- `smartContractState()`: query the wasm smart contract state
```typescript
const contractState = await rest.wasm
  .smartContractState(
...
const allContractStates = await rest.wasm.allContractState('init14mv62l7x4ducykg0crfa9a22egf8yrltmxzy84zn0wqgmr496jqs5z7k0c')
```
