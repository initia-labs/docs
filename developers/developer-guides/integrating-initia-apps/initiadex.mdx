...
<Tabs>
    <Tab title="curl">
    ```bash
    curl -X POST "[REST_URI]/initia/move/v1/accounts/0x1/modules/dex/view_functions/get_swap_simulation" \
    ...
    const restUri = 'https://....';
    const rest = new RESTClient(restUri);
...
<Tabs>
    ...
    <Tab title="InitiaJS">
    ```ts
    async function getLastProposalId(rest: RESTClient): Promise<number> {
        const [proposals, pagination] = await rest.gov.proposals()
...
async function getProposalStatus(rest: RESTClient, proposalId: number): Promise<ProposalStatus | null> {
  const proposal = await rest.gov.proposal(proposalId)
  return proposal ? proposal.status : null
}

async function checkProposalPassed(rest: RESTClient, proposalId: number): Promise<void> {
  for (;;) {
    console.log(`checking proposal ${proposalId} status... in ${rest.URL}/cosmos/gov/v1/proposals/${proposalId}`)
    const status = await getProposalStatus(rest, proposalId)
