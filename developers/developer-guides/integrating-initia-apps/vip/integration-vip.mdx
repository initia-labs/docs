...
<Steps title="VIP Integration Steps">
  <Step title="Deploy Your Rollup and Install Minitiad CLI">
    First, launch your rollup by using the [Weave CLI](/developers/developer-guides/tools/clis/weave-cli/rollup/introduction). After the rollup is live, install the [Minitiad CLI](/developers/developer-guides/tools/clis/minitiad-cli/introduction) that matches your rollup’s VM version so that later commands use the correct binary.
    <Tabs>
      <Tab title="EVM">
      ```bash
  ...
  <Step title="Create an operator and VIP contract deployer account">
    Create an operator and VIP contract deployer account that will be used to deploy the VIP scoring contract. Use the `minitiad` CLI to create a new account or import an existing one.
    ...
    `0xC5EA55D90AE84E307622F1B8B13535EC3EFF0475` is the 20-byte hex address corresponding to the bech32 address `init1ch49tkg2ap8rqa3z7xutzdf4asl07pr4nx0ps6`.
    Use this hex string whenever a contract or script expects a 0x-prefixed address rather than a bech32 one.
  </Step>
  <Step title="Deploy the VIP Scoring Contract">
    In this step, you will deploy the VIP scoring contract that tracks addresses and scores. Choose the instructions for your VM, then compile and deploy the contract with the provided scripts.
