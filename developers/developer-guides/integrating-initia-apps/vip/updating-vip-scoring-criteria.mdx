...
Changing the score criteria directly affects how much esINIT a user receives. 
...
Suppose the average score in Stage N is 1000, so the full vesting cutoff is 500. If the rollup tightens the formula in Stage N + 1 and the average drops to 10, the new cutoff falls to 5. A user who once earned 600 points may now earn only 6 for the same effort, preventing full vesting. If the scale loosens and the average climbs from 10 to 1000, users can vest earlier positions with minimal activity.
For additional information, see the [User Vesting page](home/core-concepts/vip/architecture#user-vesting).
...
<Steps>
  <Step title= "Publish a Forum Draft">
    ...
    The proposal must be submitted on the [Initia Forum's Initia Governance section](https://forum.initia.xyz/c/proposals/5) page.
