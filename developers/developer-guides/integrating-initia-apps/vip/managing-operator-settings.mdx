...
Operator commissions can be adjusted in line with the `operator_commission_max_rate` and `operator_commission_max_change_rate` settings defined during VIP whitelisting. A commission can be changed only once per stage and any updated rate takes effect starting with the next stage. 
...
`operator_commission_max_rate` limits on the operator commission. For example, if you set it to `0.1`, the operator commission can be at most 10%. This value is used to ensure that the operator commission does not exceed a certain threshold.
