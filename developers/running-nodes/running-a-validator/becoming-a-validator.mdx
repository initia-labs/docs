...
<Note>
  Before proceeding to create a validator, first make sure that your Initia
  node is fully synced to the latest height. This can be done by
...
If the command returns something, your validator is active and is
actively proposing and signing blocks.

<Note>
  Only the top 130 validators in voting power are included in the active
  validator set. This means that even if your `CreateValidator` transaction
