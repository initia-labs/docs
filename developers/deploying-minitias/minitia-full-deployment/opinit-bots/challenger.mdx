...
The challenger bot prevents misconduct and invalid minitia state submissions by monitoring for output proposals and challenging any that are invalid.

## Set Challenger Environment Variables
...
| BRIDGE_ID                   | Bridge ID                                     | ""                       |
| CHALLENGER_MNEMONIC         | The menmonic of the challenger account        | ""                       |

## Run the Challenger Bot

<CodeGroup>
```sh Node
npm run prod:challenger
```
```sh Docker
docker run -d -p 5432:5432 --network host ghcr.io/initia-labs/opinit-bots:v0.1.16 prod:challenger --env-file .env.challenger
```
</CodeGroup>