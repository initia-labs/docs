...
The `EnableAutoSignOptions` type used by
[`autoSign.enable()`](/interwovenkit/features/autosign/api-reference) is now
exported from the package, making it easier to type integrations that pass
options into the autosign enable flow.
...
stays active when the enable dialog opens. The provided value is validated
against the supported duration options (10 minutes, 1 hour, 1 day, 7 days, or
until revoked) and falls back to the standard default if it doesn't match.
