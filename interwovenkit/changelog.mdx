...
- Removed Leap from the wallet connection lists, including the Cosmos built-in
  options, the sign-in popular wallets, and the wagmi and WalletConnect
  directories.
- VIP rewards now display a chain icon for delisted apps so positions remain
  visually consistent in the portfolio.
- Improved compatibility with BigNumber 10 and later so amounts and balances
  continue to render correctly when host applications upgrade.
