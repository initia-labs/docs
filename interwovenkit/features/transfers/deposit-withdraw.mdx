...
<Tip>
  **Local Development:** Bridge modals rely on the Skip Go router to resolve
  asset paths. Since local appchains are not in the public Skip registry, these
  modals may appear blank when using a local `chainId`. For testing or demos in
  local environments, use a public testnet chain ID (e.g., `initiation-2`) to
  verify the UI.
</Tip>
