...
`InterwovenKitProvider`. InterwovenKit handles ghost wallet creation and
management automatically—no external wallet provider setup is required.
...
```tsx
<InterwovenKitProvider enableAutoSign>
  {children}
</InterwovenKitProvider>
```
