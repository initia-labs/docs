...
<ParamField path="expiredAtByChain" type="Record<string, Date | null | undefined>">
  ...
  - `Date`: The date when autosign expires
  - `null`: Autosign is not enabled for this chain
  - `undefined`: Autosign is enabled with no expiration (permanent)
</ParamField>
...
Map of chain IDs to grantee addresses (ghost wallet addresses).
      ...
      {Object.entries(autoSign.isEnabledByChain).map(([chainId, isEnabled]) => {
        ...
        return (
          <div key={chainId}>
            ...
            {isEnabled && expiration && ` (until ${expiration.toLocaleString()})`}
          </div>
        )
      })}
    </div>
