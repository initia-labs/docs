...
wallet (also known as a "ghost wallet") to sign transactions on behalf of users,
providing a seamless user experience while maintaining security through granular
permission controls.
...
Privy creates an embedded wallet (ghost wallet) that will sign transactions on
the user's behalf. This wallet is created automatically when the user enables
autosign.
