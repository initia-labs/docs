...
Ghost wallets are Privy-managed embedded wallets that act as authorized signers for your application. When autosign is enabled, Privy creates a ghost wallet that receives permission from the user's main wallet to sign specific transaction types automatically.
