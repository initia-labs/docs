...
title: Introduction
description:
  Learn about autosign and dedicated signing wallets for seamless transaction
  signing.
---
...
transactions on your behalf, providing a seamless experience while maintaining
security through granular permission controls.
...
that derived wallet as the grantee for the authz and feegrant permissions needed
to sign approved transaction types automatically.
...
- **Revocable**: You can revoke permissions at any time
...
- **Authz grants**: Authorize the AutoSign wallet to execute specific message
  types
- **Feegrant**: Allows the AutoSign wallet to pay transaction fees on behalf of
  the user
...
- **Control**: You can see and manage all autosign permissions
...
- **Scoped Permissions**: Only specific message types can be auto-signed. For
  ...
  `/cosmos.bank.v1beta1.MsgSend`, ensuring the AutoSign wallet can only execute
  the exact operations you've authorized.
- **Time-Limited Grants**: All autosign permissions have expiration dates. You
...
- **Revocable Permissions**: You can revoke autosign permissions at any time
  through your wallet settings. When revoked, all grants are immediately
  invalidated.
