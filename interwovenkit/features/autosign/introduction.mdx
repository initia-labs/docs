---
title: Introduction
description: Learn about autosign and ghost wallets for seamless transaction signing.
icon: robot
---
...
<Info>
  Autosign is built on the Cosmos SDK's `authz` and `feegrant` modules, allowing fine-grained control over which
  transaction types can be automatically signed and when permissions expire.
</Info>
