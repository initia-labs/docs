...
To enable autosign, call the `enable()` method. This opens a drawer where you
...
<Step title="Drawer Opens">
  A drawer appears asking you to confirm autosign setup, showing which permissions will be granted.
</Step>

<Step title="Expiration Selection">
  You can select an expiration duration from available defaults or use a custom
  duration.
</Step>
...
<Step title="Permission Grant">
  You sign a single transaction to grant `authz` and `feegrant` permissions to
  the AutoSign wallet.
</Step>
...
You can disable autosign at any time. This revokes all grants for the selected
