...
Once autosign is enabled, you can send transactions without user confirmation using `submitTxBlock`. This method signs and broadcasts transactions directly using the ghost wallet.
...
function SendWithAutosign() {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async () => {
      ...
      // Submit transaction directly (no modal, uses ghost wallet)
...
<Tip>
Use `submitTxBlock` for seamless transactions with autosign. For transactions requiring user confirmation, use `requestTxBlock` instead.
</Tip>
