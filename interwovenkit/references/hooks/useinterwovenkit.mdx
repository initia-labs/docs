...
<Tip>
  **InterwovenKit UI methods**: Use `requestTxSync` and `requestTxBlock` for applications using InterwovenKit's fee selection modal.
  **Direct wallet methods**: Use `submitTxSync` and `submitTxBlock` for custom transaction UX that skips InterwovenKit's modal and goes directly to browser wallet confirmation.
</Tip>
