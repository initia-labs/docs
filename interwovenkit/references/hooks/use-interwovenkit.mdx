...
<ParamField path="address" type="string">
  ...
  chain type (hex for `minievm`, bech32 for others).
</ParamField>
...
>
  ...
  transaction response. Shows transaction approval UI before signing.
</ParamField>
...
>
  ...
  returning the complete transaction response. Does not show UI.
</ParamField>
...
<ParamField path="internal" type="boolean | string | number">
...
<ParamField path="intervalMs" type="number">
  Polling interval in milliseconds for checking transaction status.
</ParamField>
...
<ParamField path="autoSign.enable" type="(chainId?: string) => Promise<void>">
  ...
  enables for all supported chains if omitted.
</ParamField>

<ParamField path="autoSign.disable" type="(chainId?: string) => Promise<void>">
  Disables AutoSign for a chain. Optionally specify a chain ID, or disables for
