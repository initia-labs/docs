...
<Steps>
  ...
  <Step title="Custom Fee Handling (Optional)">
    The custom fee handling feature allows you to bypass the "Confirm tx" modal and provide pre-calculated fees directly, giving you more control over transaction speed and UX. The old requestTx functions are still available, so you can choose the workflow that fits your app best.
