...
Setting `INTERNAL_TX=true` for Move or Wasm rollups is not supported and will fail.

### EVM ret cleanup (optional)

If your EVM database was indexed before the ret-only address fix, enable the cleanup extension to remove incorrect `tx_accounts` entries:
