 ...
 */
export async function opDenomByL1Denom(uri: string, bridge_id: number, l1_denom: string): Promise<string> {
 ...
 */
export async function opDenomByL2Denom(uri: string, bridge_id: number, l2_denom: string): Promise<string> {
  const restClient = new RESTClient(uri)
...
async function getFullTrace(restClient: RESTClient, denom: string): Promise<string> {
  ...
  const trace = await restClient.apiRequester.get<DenomTrace>(`/ibc/apps/transfer/v1/denom_traces/${denom}`)
...
export async function ibcDenom(
  ...
  const denomInChainB = fullTraceOnChainB.startsWith('transfer/') ? makeIBCHash(fullTraceOnChainB) : fullTraceOnChainB
